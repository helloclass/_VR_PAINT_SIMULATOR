...
#pragma warning restore 0649
		...
		public bool PaintUVDirect(Brush brush, Vector2 uv, Func<PaintSet, bool> materialSelector = null)
		{
            ...
            uv += new Vector2(x * brush.getBrushScatter(), y * brush.getBrushScatter()) ;
				...
				if(EditorApplication.isPlaying)
				{
					...
					for(int i = 0; i < instance.paintSet.Count; ++i)
					{
						if(foldOut[i] = Foldout(foldOut[i], string.Format("Material \"{0}\"", materials[i].name)))
						{
                            ...
                            if (paintSet.paintMainTexture != null && GUILayout.Button("Save main texture") || (paintSet.paintMainTexture != null && GameObject.Find("SaveButton").GetComponent<SaveBtn>().isClicked))
                            {
                                Debug.Log("앙뇽?");
                                SaveRenderTextureToPNG(paintSet.mainTexture != null ? paintSet.mainTexture.name : "main_texture", paintSet.paintMainTexture);
                                GameObject.Find("SaveButton").GetComponent<SaveBtn>().isClicked = false;
                            }
                            else
                            {
                                Debug.Log("ㅜㅜ " + paintSet.paintMainTexture + ", " + GameObject.Find("SaveButton").GetComponent<SaveBtn>().isClicked);
                            }
